...
def CCDIK(EndInd, InOutChain, AngleDelta, TargetPosition, Precision, MaxIteration, bEnableRotationLimit, RotationLimitPerJoints):
    ...
    def UpdateChainLink(Chain, InAngleDelta, LinkIndex, TargetPos, bInEnableRotationLimit, InRotationLimitPerJoints):
        ...
        bCanRotate = bInEnableRotationLimit == False or RotationLimitPerJointInRadian > np.abs(
            InAngleDelta[LinkIndex])  # AngleDelta[LinkIndex] represents "CurrentLink.CurrentAngleDelta"
        if bCanRotate:
            ...
            if bInEnableRotationLimit:
                if RotationAxis[0] > 0:
                    if RotationLimitPerJointInRadian < Angle + InAngleDelta[LinkIndex]:
                        # Limitation - CurrentAngle -> Toward Maximum Angle # -0.001 to avoid lock
                        Angle = RotationLimitPerJointInRadian - \
                            InAngleDelta[LinkIndex]-0.001
                    InAngleDelta[LinkIndex] += Angle
                elif RotationAxis[0] < 0:
                    if RotationLimitPerJointInRadian * -1 > InAngleDelta[LinkIndex] - Angle:
                        Angle = RotationLimitPerJointInRadian + \
                            InAngleDelta[LinkIndex]-0.001
                    InAngleDelta[LinkIndex] -= Angle
            ...
            Chain = SetRotation(Chain, LinkIndex, NewRotation)
            updated = True
        else:
            updated = False
        return Chain, updated, InAngleDelta
...
class Point_Move:
    ...
    def __init__(self):
        ...
        self.RotationLimitPerJoints = np.ones_like(self.angleDelta) * 60.
        self.RotationLimitPerJoints[0] = 360.
        # line3D
    ...
    def motion_notify_callback(self, event):
        ...
        self.chain, self.angleDelta = CCDIK(self._ind, self.chain, self.angleDelta, np.asarray(
            [x_out, y_out, z_out]), 0.01, 10, True, self.RotationLimitPerJoints)
