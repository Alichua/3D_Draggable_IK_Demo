'''
An 3D draggable module, no IK implemented.
'''
# 3d dependancy
...
class Point_Move:
    ...
    def motion_notify_callback(self, event):
        ...
        x_dist, y_dist, z_dist = x_out - \
            self.x[self._ind], y_out - \
            self.y[self._ind], z_out - self.z[self._ind]
        self.x[self._ind] = x_out
        self.y[self._ind] = y_out
        self.z[self._ind] = z_out
        # TODO: add IK
        # linked joint (fake IK)
        self.x[self._ind-1] += x_dist
        self.y[self._ind-1] += y_dist
        self.z[self._ind-1] += z_dist
